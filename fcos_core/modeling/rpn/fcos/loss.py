...
class FCOSLossComputation(object):
    ...
    def compute_features_targets(self, features, locations, targets):
       ...
       for i,f in enumerate(features):#i = (5 feature maps 0,1,2,3,4)p3,p4,p5,p6,p7
          ...
          for j in range(len(targets)):#targets = batch_image 0,1,2,3
            ...
            bboxes = targets_per_im.bbox
            p = -1
            #print(weights.shape)#([52, 64]) ([26, 32])
            ...
            if i!=5 :
              ...
              for k in range(weights.shape[0]):
                ...
                #print("size:",sizes)#(499,354)
                  ...
                  box1 = np.array(box1)
                  #print(i,box1,len(locations[i]))
                #print("box1:",p,box1,len(locations[i]))
                  for l in range(len(bboxes)):
                    box2 = [(bboxes[l])[0].cpu(),(bboxes[l])[1].cpu(),(bboxes[l])[2].cpu(),(bboxes[l])[3].cpu()]
                    #box2 = [(bboxes[l])[0].to(device),(bboxes[l])[1].to(device),(bboxes[l])[2].to(device),(bboxes[l])[3].to(device)]
                    box2 = np.array(box2)
                    if IOU(box1,box2)!=0:
                      weights[k][h] +=3
                  weights[k][h] = torch.sigmoid(weights[k][h])
              saves_feature_weight.append(weights)
              #print(saves_feature_weight[0].shape)
          if len(saves_feature_weight)!=0:
